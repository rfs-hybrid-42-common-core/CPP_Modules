/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   PresidentialPardonForm.cpp                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: maaugust <maaugust@student.42porto.com>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/08/09 02:02:43 by maaugust          #+#    #+#             */
/*   Updated: 2026/08/10 02:10:52 by maaugust         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/* ========================================================================== */
/* INCLUDES                                                                   */
/* ========================================================================== */

/* --------------------------- External Libraries --------------------------- */
#include "PresidentialPardonForm.hpp"

/* --------------------------- Internal Libraries --------------------------- */
#include <iostream>

/* ========================================================================== */
/* PRIVATE METHODS                                                            */
/* ========================================================================== */

/**
 * @fn void PresidentialPardonForm::executeAction(void) const
 * @brief Executes the specific action of the Presidential form.
 * @details Prints a message to the standard output announcing that the target 
 * has been officially pardoned by Zaphod Beeblebrox.
 */
void PresidentialPardonForm::executeAction(void) const
{
    std::cout << "Congratulations, " << getTarget()
              << "! You have just been granted an official pardon by the magnanimous Zaphod Beeblebrox." << std::endl;
}

/* ========================================================================== */
/* CONSTRUCTORS & DESTRUCTORS                                                 */
/* ========================================================================== */

/**
 * @fn PresidentialPardonForm::PresidentialPardonForm(void)
 * @brief Default constructor for the PresidentialPardonForm class.
 * @details Initializes the base AForm with strict Presidential grades 
 * (Sign 25, Exec 5) and sets a default target of "president".
 */
PresidentialPardonForm::PresidentialPardonForm(void) :
    AForm("Presidential Pardon Form", _signGrade, _execGrade),
    _target("president")
{
    std::cout << getName() << " was successfully generated. Its destination target is " << getTarget() << "." << std::endl;
}

/**
 * @fn PresidentialPardonForm::PresidentialPardonForm(std::string const & target)
 * @brief Parameterized constructor for the PresidentialPardonForm class.
 * @details Initializes the base AForm with strict Presidential grades and 
 * assigns the specific target for the pardon.
 * @param target The string representing the target to be pardoned.
 */
PresidentialPardonForm::PresidentialPardonForm(std::string const & target) :
    AForm("Presidential Pardon Form", _signGrade, _execGrade),
    _target(target)
{
    std::cout << getName() << " was successfully generated. Its destination target is " << getTarget() << "." << std::endl;
}

/**
 * @fn PresidentialPardonForm::PresidentialPardonForm(PresidentialPardonForm const & other)
 * @brief Copy constructor for the PresidentialPardonForm class.
 * @details Constructs a new object by deeply copying the base AForm properties 
 * and the specific _target attribute.
 * @param other The PresidentialPardonForm object to copy from.
 */
PresidentialPardonForm::PresidentialPardonForm(PresidentialPardonForm const & other) :
    AForm(other),
    _target(other.getTarget())
{
    std::cout << getName() << " was successfully duplicated. Its destination target is " << getTarget() << "." << std::endl;
}

/**
 * @fn PresidentialPardonForm::~PresidentialPardonForm(void)
 * @brief Destructor for the PresidentialPardonForm class.
 * @details Safely destroys the form.
 */
PresidentialPardonForm::~PresidentialPardonForm(void)
{
    std::cout << getName() << " has fulfilled its purpose and is no longer needed." << std::endl;
}

/* ========================================================================== */
/* GETTERS                                                                    */
/* ========================================================================== */

/**
 * @fn std::string const & PresidentialPardonForm::getTarget(void) const
 * @brief Retrieves the form's target.
 * @details Provides read-only access to the target string attribute.
 * @return A constant reference to the target string.
 */
std::string const & PresidentialPardonForm::getTarget(void) const
{
    return _target;
}

/* ========================================================================== */
/* OPERATOR OVERLOADS                                                         */
/* ========================================================================== */

/**
 * @fn PresidentialPardonForm& PresidentialPardonForm::operator=(PresidentialPardonForm const & other)
 * @brief Copy assignment operator.
 * @details Safely calls the base class assignment operator to copy the 
 * signature status, then assigns the specific _target. Protects against 
 * self-assignment.
 * @param other The PresidentialPardonForm to assign from.
 * @return      A reference to the newly updated object.
 */
PresidentialPardonForm& PresidentialPardonForm::operator=(PresidentialPardonForm const & other)
{
    if (this != &other)
    {
        AForm::operator=(other);
        if (getTarget() != other.getTarget())
        {
            _target = other.getTarget();
            std::cout << getName() << " new target is " << getTarget() << "." << std::endl;
        }
        else
            std::cout << "The target is still " << getTarget() << "." << std::endl;
    }
    else
        std::cout << "You provided the same " << getName() << ". No changes were applied." << std::endl;
    return *this;
}
